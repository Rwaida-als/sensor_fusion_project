...
def compute_cpts():
 ...
 # Define all possible Wall Distance Categories
    ...
    cpt_bumps = (cpt_bumps * (cpt_bumps.sum(axis=1).values.reshape(-1,1)) + 1) / (cpt_bumps.sum$
    
    # Ensure all bump categories are present in the CPT
 cpt_bumps = cpt_bumps.reindex(columns=bump_categories, fill_value=1/(cpt_bumps.sum(axis=1) $
    
    # Normalize rows to ensure they sum to 1
    cpt_bumps = cpt_bumps.div(cpt_bumps.sum(axis=1), axis=0)
    
    # Replace NaN with 0 (in case any row had all zeros)
    cpt_bumps = cpt_bumps.fillna(0)
...
counts_wall = df.groupby(['IR Value Category', 'Door Detected'])['Wall Distance Category'].$
    ...
    # Replace NaN with 0 (in case any row had all zeros)
    cpt_wall_distance = cpt_wall_distance.fillna(0)
